...
void Address::choose_Region() {
	...
	QSqlQueryModel* model = new QSqlQueryModel(this);
        QSqlQuery query;
        query.prepare("SELECT  CODE, CODE1, NAME FROM KLADR WHERE (RTYPE = 1) ORDER BY CODE1");
        if(!query.exec()){
	...
	if (dialog->result() == QDialog::Accepted) {
		...
		bool ok;
		code = rec.field("CODE").value().toLongLong(&ok);
...
void Address::choose_City() {
	...
	if (dialog->result() == QDialog::Accepted) {
		...
		bool ok;
                ...
                code1 = rec.field("CODE1").value().toInt(&ok);
                code2 = rec.field("CODE2").value().toInt(&ok);
                code3 = rec.field("CODE3").value().toLongLong(&ok);
		QString name = rec.field("NAME").value().toString();
